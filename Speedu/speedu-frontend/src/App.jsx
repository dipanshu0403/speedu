...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50/80 text-slate-900 antialiased">
      <Navbar token={app.token} role={app.role} go={app.go} logout={app.logout} />
      ...
      {app.view === "home" && (
        <Home
          ...
          services={app.filteredServices}
          openService={app.openService}
          loadServices={app.loadServices}
        />
      )}
      {app.view === "auth" && (
        <Auth
      ...
      {app.view === "service" && (
        <Service
          ...
          selectedVariant={app.selectedVariant}
          setSelectedVariant={app.setSelectedVariant}
          addresses={app.addresses}
          token={app.token}
          role={app.role}
          go={app.go}
          createBooking={app.createBooking}
        />
      )}
      {app.view === "payment" && (
        <Payment booking={app.draftBooking} loading={app.loading} createPayment={app.createPayment} />
      )}
      {app.view === "paymentSuccess" && <PaymentSuccess payment={app.paymentResult} go={app.go} />}
      {app.view === "bookings" && (
        <Bookings bookings={app.bookings} loadBookings={app.loadBookings} updateBooking={app.updateBooking} />
      )}
      {app.view === "agent" && (
        <Agent
          bookings={app.agentBookings}
          profileId={app.profileId}
          loadBookings={app.loadBookings}
          updateBooking={app.updateBooking}
        />
      )}
      {app.view === "admin" && (
        <Admin
          services={app.services}
          bookings={app.adminBookings}
          payments={app.payments}
          loading={app.loading}
          createService={app.createService}
          createVariant={app.createVariant}
          removeService={app.removeService}
          removeVariant={app.removeVariant}
          loadAdminData={app.loadAdminData}
